...
def sample_trajectory(
    ...
    while True:
        ...
        # [x] use the most recent ob and the policy to decide what to do
        # get action from policy
        ac: np.ndarray = policy.get_action(ob)

        # [x]: use that action to take a step in the environment
        # env step(ac) -> next_ob, rew, done, _
        next_ob, rew, done, _ = env.step(ac)

        # [x] rollout can end due to done, or due to max_length
        steps += 1
